...
import { User, ArrowLeft, ShieldCheck, Mail, ChevronRight, Zap, Globe, CheckCircle2, HeartPulse } from "lucide-react";
...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        setSubmitted(true);
        toast.success("Identity verification request initiated.");
      } else {
        ...
        <div className="relative z-10 flex flex-col h-full justify-between">
            ...
            <div className="max-w-[90%]">
               <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                  <p className="text-white/60 font-black text-[12px] tracking-[5px] mb-4">Security Protocol</p>
                  <h1 className="text-white text-[56px] font-bold leading-[1] mb-6 tracking-tighter">
                    Account <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-indigo-100">Recovery.</span>
                  </h1>
                  <p className="text-white/70 text-[18px] leading-relaxed font-medium mb-10 max-w-[600px]">
                    Initiate a secure handshake to verify your identity and regain access to the institutional node.
                  </p>
               </motion.div>
            </div>
            ...
            <div className="flex items-center justify-between border-t border-white/10 pt-8">
               <div className="flex items-center gap-4">
                  ...
                  <p className="text-white/80 text-[14px] font-bold">Encrypted Handshake Protocol</p>
               </div>
            </div>
        </div>
      </div>
        ...
        <div className="min-h-full flex flex-col items-center justify-center p-6 py-16 md:p-12">
          ...
          {!submitted ? (
            <>
              <div className="mb-10 text-left">
                <h2 className="text-[26px] font-bold text-[#1e293b] dark:text-white leading-tight">Recover <br/>Access Token.</h2>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
                      <Mail className="w-[17px] h-[17px] stroke-[2]" />
                    </div>
                    <input
                      type="email"
                      required
                ...
                >
                  {isLoading ? (
                    ...
                    <span className="flex items-center justify-center gap-2">
                      Send Reset Instructions <ChevronRight className="w-4 h-4 ml-1" />
                    </span>
                  )}
                </button>
              </form>
            ...
            <div className="text-center">
              ...
              <h2 className="text-[24px] font-bold text-[#1e293b] dark:text-white leading-tight mb-4">Verification Sent.</h2>
              <p className="text-[14px] font-semibold text-gray-500 leading-relaxed mb-8">
                If an account exists for <span className="text-[#1e293b] dark:text-white font-black">{email}</span>, you will receive instructions shortly.
              </p>
              
              <Link 
                href="/auth/login" 
                className="inline-flex items-center justify-center w-full h-[52px] rounded-[5px] border border-[#E7E8EB] dark:border-white/10 text-[13px] font-bold text-[#1e293b] dark:text-white hover:bg-gray-50 transition-all"
              >
                Back to Login
              </Link>
            </div>
          ...
          <div className="mt-12 w-full flex justify-between items-center text-[10px] font-bold text-gray-400 border-t border-gray-100 dark:border-white/5 pt-6">
             ...
             <div className="flex items-center gap-6">
                ...
                <a href="#" className="hover:text-[#2E37A4]">Contact Node</a>
             </div>
          </div>
          </div>
        </div>
