          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {departments.map((dept, i) => (
              <button
                ...
                className={cn(
                  "flex flex-col items-center justify-center py-5 sm:py-8 px-2 sm:px-4 gap-2 sm:gap-3 rounded-[var(--radius)] border transition-all relative shadow-none outline-none group",
              ...
              >
                ...
                <span className={cn(
                  "text-[11px] sm:text-[13px] font-medium transition-all text-center",
        ...
        <div className="bg-card border border-border rounded-[var(--radius)] p-4 sm:p-6 space-y-4 sm:space-y-5 shadow-none">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full h-11 pl-11 pr-4 bg-background border border-border rounded-[var(--radius)] text-[13px] font-medium outline-none focus:border-primary transition-all shadow-none"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              </div>
              <button 
                className="w-full sm:w-auto px-8 bg-primary text-white rounded-[var(--radius)] text-[13px] font-bold hover:bg-primary/90 transition-all shadow-none h-11"
              >
                Search
              </button>
            </div>
            ...
            <div className="flex items-center justify-between gap-2">
              <p className="text-[10px] sm:text-[11px] font-bold text-muted-foreground/40 tracking-tight uppercase">Search Results</p>
              <button 
                onClick={() => router.push("/reception/patient-registration/new")}
                className="text-[11px] sm:text-[12px] font-bold text-primary hover:underline whitespace-nowrap"
              >
            ...
            <div className="space-y-3">
              {showResults ? (
                <div className="p-4 border border-border bg-card rounded-[var(--radius)] relative overflow-hidden group cursor-pointer transition-all border-l-[3px] border-l-primary">
                  <div className="flex justify-between items-start text-left">
                    ...
                    <span className="text-[10px] font-bold text-muted-foreground/60 bg-muted px-2 py-0.5 rounded-[2px]">UHID: 102938</span>
                  </div>
                </div>
              ) : (
                <div className="h-32 border border-border border-dashed rounded-[var(--radius)] flex items-center justify-center bg-muted/20">
