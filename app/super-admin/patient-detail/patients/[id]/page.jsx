...
export default function PatientDetailsPage({ params }) {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      try {
        ...
        const url = `/api/patients/${resolvedParams.id}${branchId ? `?branchId=${branchId}` : ""}`;
  ...
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    router.push(`?tab=${tab}${branchId ? `&branchId=${branchId}` : ""}`, { scroll: false });
  };
        ...
        return (
          <div className="space-y-5">
            <div className="flex justify-between items-center">
              ...
              <button 
                onClick={() => router.push(`/super-admin/patient-detail/vitals/${resolvedParams.id}${branchId ? `?branchId=${branchId}` : ""}`)}
            ...
            <VitalsHistory patientId={resolvedParams.id} branchId={branchId} />
          </div>
        );
      case "Medication":
        ...
        return (
          <div className="space-y-5">
            <div className="flex justify-between items-center">
              ...
              <button 
                onClick={() => router.push(`/super-admin/patient-detail/tasks${branchId ? `?branchId=${branchId}` : ""}`)}
                className="bg-primary text-primary-foreground px-6 h-[40px] rounded-lg text-[12px] font-bold flex items-center justify-center gap-2 hover:bg-primary/90 transition-all shadow-none"
              >
                <Plus className="w-4 h-4" />
                Add Tasks
            ...
            <NursingTasks patientId={resolvedParams.id} branchId={branchId} />
          </div>
        );
      case "Lab Results":
        return (
