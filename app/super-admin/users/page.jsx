...
export default function UserManagementPage() {
  ...
  const handleDelete = async (user) => {
    const confirmed = window.confirm(`Are you sure you want to delete ${user.name}? This will remove them from all branches.`);
  ...
  return (
    <div className="p-6 bg-[#F8F9FC] dark:bg-[#0A0F1D] min-h-screen flex flex-col transition-colors duration-300 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-5">
        <h1 className="text-[20px] font-bold text-[#1e293b] dark:text-white tracking-tight leading-none">User Management</h1>
      ...
      <div className="space-y-[30px] pb-20 relative">
        ...
        {loading && (
          <div className="absolute inset-0 top-32 flex flex-col items-center pt-24 bg-white/50 dark:bg-[#0A0F1D]/50 backdrop-blur-sm z-10 rounded-[5px]">
            ...
            <p className="text-[11px] font-bold text-gray-400">Loading users...</p>
          </div>
        )}
      </div>
