  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className="h-11 px-5 bg-background border border-border rounded-[5px] text-[13px] font-bold text-foreground flex items-center justify-between gap-3 hover:bg-muted transition-all outline-none w-full sm:w-auto shadow-none focus:border-primary"
          style={{ minWidth }}
        >
      ...
      <DropdownMenuContent
        align="end"
        className="w-[calc(100vw-48px)] sm:w-[var(--radix-dropdown-menu-trigger-width)] sm:min-w-[140px] border border-border bg-card p-1 rounded-[5px] shadow-xl z-[500]"
      >
        <DropdownMenuItem
          onClick={() => onChange("")}
          className={cn(
            "rounded-[5px] text-[12px] font-medium px-3 py-2 cursor-pointer transition-colors outline-none",
...
function StatCard({ label, value, icon: Icon, color }) {
  return (
    <div className="bg-card p-5 rounded-[5px] border border-border flex items-center gap-4 shadow-none">
      <div
        className={cn(
      ...
      <div>
        <p className="text-[11px] font-bold text-muted-foreground leading-none mb-1.5 tracking-tight">
          {label}
        </p>
  ...
  return (
    <div className="p-[20px] bg-background min-h-screen flex flex-col space-y-[20px] transition-colors duration-300 font-sans pb-20">
      ...
      <div className="space-y-[20px]">
        ...
        <div className="flex flex-col xl:flex-row justify-between items-stretch xl:items-center gap-4 bg-card p-3 rounded-[5px] border border-border shadow-none">
          <div className="relative w-full xl:w-[380px]">
            ...
            <input
              type="text"
              placeholder="Search..."
              className="w-full h-11 pl-11 pr-4 bg-background border border-border rounded-[5px] text-[13px] font-medium focus:border-primary transition-all outline-none text-foreground placeholder:text-muted-foreground shadow-none"
              value={searchQuery}
        ...
        {loading ? (
          <div className="p-10 flex justify-center text-muted-foreground bg-card rounded-[5px] border border-border shadow-none">
            <div className="animate-spin w-6 h-6 border-2 border-primary border-t-transparent rounded-full mr-3"></div> Loading...
          </div>
        ) : filtered.length === 0 ? (
          <div className="p-10 flex justify-center items-center flex-col text-muted-foreground bg-card rounded-[5px] border border-border shadow-none">
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-4 md:hidden">
              {filtered.map((v) => (
                <div
                  key={v.id}
                  className="bg-card p-5 rounded-[5px] border border-border active:scale-[0.98] transition-all shadow-none"
                >
                  {/* Card Header */}
                  ...
                  <div className="grid grid-cols-3 gap-x-4 gap-y-4 mb-4">
                    {[
                      ...
                      <div key={label}>
                        <p className="text-[10px] font-bold text-muted-foreground tracking-tight mb-1">{label}</p>
                        <p className={cn(
                          "text-[13px] font-bold",
            ...
            {/* ── Desktop Table View ── */}
            <div className="hidden md:block bg-card rounded-[5px] border border-border shadow-none overflow-hidden">
              <div className="overflow-x-auto no-scrollbar">
                <table className="w-full border-collapse">
                    ...
                    <tr className="bg-muted/30">
                      {["Patient Name", "BP (mmHg)", "HR", "SpO2", "Temp", "Resp", "Pain", "Recorded By", "Actions"].map((col, i) => (
                        <th
                          key={i}
                          className={cn(
                            "px-8 py-3 text-[11px] font-bold text-muted-foreground border-b border-border tracking-tight",
                  ...
                  <tbody className="divide-y divide-border">
                    {filtered.map((v) => (
                      <tr key={v.id} className="hover:bg-muted/10 transition-all">
                        ...
                        <td className="px-8 py-3">
                          {v.bpStatus === "critical" ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-[5px] bg-destructive/10 border border-destructive/20 text-destructive text-[12px] font-bold">
                              <AlertTriangle className="w-3 h-3 shrink-0" />{v.bp}
                            </span>
                          ) : v.bpStatus === "abnormal" ? (
                            <span className="inline-flex items-center px-2.5 py-1 rounded-[5px] bg-amber-500/10 border border-amber-500/20 text-amber-500 text-[12px] font-bold">
                              {v.bp}
                            </span>
                          ) : (
                        ...
                        <td className="px-8 py-3">
                          {v.spo2Status === "critical" ? (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-[5px] bg-destructive/10 border border-destructive/20 text-destructive text-[12px] font-bold">
                              ↓ {v.spo2}
                            </span>
