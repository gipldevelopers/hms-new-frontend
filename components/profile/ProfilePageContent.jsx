...
import { ShieldCheck, EyeOff, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function ProfilePageContent({ userRole = "Staff", userName = "User Name", userId = "ID-0000", userDept = "General" }) {
  ...
  return (
    <div className="p-4 md:p-5 bg-background min-h-screen flex flex-col gap-5 md:gap-6 transition-colors duration-300 font-sans pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        ...
        {/* Success Alert */}
        {showSuccess && (
          <div className="bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20 px-4 py-2 rounded-lg flex items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-300 w-full sm:w-auto">
      ...
      <div className="bg-card border border-border rounded-lg p-6 md:p-8 flex flex-col md:flex-row items-center md:items-start gap-8 shadow-none">
        ...
        <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6">
          ...
          <div className="space-y-2">
            <label className="text-[12px] font-bold text-muted-foreground uppercase tracking-wider">Role / Designation</label>
            <div className="h-11 px-4 bg-muted/20 border border-border rounded-lg flex items-center text-[13px] font-medium text-foreground cursor-not-allowed uppercase">
              {userRole}
            </div>
          </div>
          <div className="space-y-2">
