...
const ActiveDropdownItem = React.forwardRef(({ children, active, onClick, ...props }, ref) => {
  const localRef = React.useRef(null);
  const combinedRef = ref || localRef;

  React.useEffect(() => {
    if (active && combinedRef.current) {
      const timer = setTimeout(() => {
        combinedRef.current?.scrollIntoView({ block: "center", behavior: "instant" });
      }, 50);
  ...
  return (
    <DropdownMenuItem
      ...
      onClick={onClick}
      className={cn(
        "text-[12px] font-bold py-1.5 px-3 rounded-[5px] cursor-pointer hover:bg-muted text-foreground",
...
ActiveDropdownItem.displayName = "ActiveDropdownItem";
      ...
      <div className="flex justify-between items-center mb-4 px-1">
        ...
        <div className="flex items-center gap-1">
          {mode !== "month-year" && (
            <>
              <DropdownMenu>
                ...
                <DropdownMenuContent className="max-h-[220px] overflow-y-auto z-[150] w-[120px]">
                  {months.map((m, i) => (
                    <ActiveDropdownItem
                      key={m}
                      active={i === month}
                      onClick={() => setCurrentMonth(new Date(year, i))}
                    >
                      {m}
                    </ActiveDropdownItem>
                  ))}
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="max-h-[220px] overflow-y-auto z-[150] w-[100px] no-scrollbar">
              {years.map((y) => (
                <ActiveDropdownItem
                  key={y}
                  active={y === year}
                  onClick={() => setCurrentMonth(new Date(y, month))}
                >
                  {y}
                </ActiveDropdownItem>
              ))}
