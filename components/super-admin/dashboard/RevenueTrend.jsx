  ...
  return (
    <div className="w-full bg-white dark:bg-[#101935] rounded-[5px] border border-[#E7E8EB] dark:border-white/10 flex flex-col font-sans transition-all">
      ...
      <div className="flex justify-between items-center p-5">
        ...
        <button className="flex items-center gap-2 text-[12px] font-medium text-[#64748B] dark:text-slate-400 border border-[#E7E8EB] dark:border-white/10 px-3 py-1.5 rounded-[5px] hover:bg-gray-50 dark:hover:bg-[#1e293b] transition-colors">
          Monthly <ChevronDown className="w-3.5 h-3.5" />
        </button>
      </div>

      <div className="px-6 pb-12 pt-2 relative">
        {/* Y-Axis Labels & Dashed Lines */}
        <div className="absolute inset-x-6 top-2 bottom-20 flex flex-col justify-between pointer-events-none">
          {["5L", "4L", "3L", "2L", "1L", "0L"].map((label) => (
            <div key={label} className="relative flex items-center w-full h-0">
              <span className="absolute -left-1 text-[12px] font-medium text-[#94A3B8] dark:text-slate-500 w-6 text-right">
                {label}
              </span>
        ...
        {/* Bars Container */}
        <div className="h-[240px] ml-8 flex items-end justify-between relative z-10 px-2">
          {data.map((item, i) => (
            <div
              key={i}
            ...
            >
              ...
              {/* X-Axis Month Label */}
              <span className="absolute -bottom-8 text-[12px] text-[#64748B] dark:text-slate-500 font-medium">
                {item.m}
              </span>
            </div>
          ))}
