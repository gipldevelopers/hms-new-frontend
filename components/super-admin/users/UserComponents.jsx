...
export function UserStats() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {cards.map((stat, i) => (
        <div key={i} className="bg-card p-5 rounded-[5px] border border-border flex items-center gap-4 transition-all">
          <div className={cn(
          ...
          <div>
            <p className="text-[11px] font-bold text-muted-foreground leading-none mb-1.5">{stat.title}</p>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-card p-3 rounded-[5px] border border-border shadow-none">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-[200px] border-border bg-card shadow-xl max-h-64 overflow-y-auto rounded-[5px] p-1">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-muted/30">
                <th className="px-8 py-3 text-left text-[11px] font-bold text-muted-foreground border-b border-border">Name</th>
                <th className="px-8 py-3 text-left text-[11px] font-bold text-muted-foreground border-b border-border">Email</th>
                <th className="px-8 py-3 text-left text-[11px] font-bold text-muted-foreground border-b border-border">Role</th>
                <th className="px-8 py-3 text-left text-[11px] font-bold text-muted-foreground border-b border-border">Assigned hospital</th>
                <th className="px-8 py-3 text-left text-[11px] font-bold text-muted-foreground border-b border-border">Status</th>
                <th className="px-8 py-3 text-right text-[11px] font-bold text-muted-foreground border-b border-border">Actions</th>
              </tr>
            </thead>
                ...
                users.map((user, i) => (
                  <tr key={i} className="hover:bg-muted/10 transition-all group">
                    <td className="px-8 py-3">
                      <div className="text-[14px] font-bold text-foreground leading-tight">{user.name}</div>
                    </td>
                    <td className="px-8 py-3">
                      <div className="text-[14px] text-muted-foreground font-medium">{user.email}</div>
                    </td>
                    <td className="px-8 py-3">
                      <div className="text-[14px] font-bold text-foreground">{formatRole(user.role)}</div>
                    </td>
                    <td className="px-8 py-3">
                      <div className="text-[14px] text-muted-foreground font-medium">{user.hospital}</div>
                    </td>
                    <td className="px-8 py-3">
                      <span className={cn(
