  ...
  return (
    <div className="space-y-[20px] animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-[15px] md:text-[18px] font-bold text-foreground uppercase tracking-widest leading-none">Active Orders</h3>
      </div>
      ...
      <div className="space-y-4 pb-10">
        {filteredOrders.map((order, idx) => (
          <div 
            key={idx} 
            className={cn(
              "bg-card p-5 md:p-6 rounded-lg border transition-all shadow-none",
              order.isUrgent ? "border-destructive/30 ring-1 ring-destructive/10" : "border-border"
            )}
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div className="flex flex-wrap items-center gap-3 md:gap-4">
                <span className={cn(
                  ...
                  order.categoryVariant === "danger" ? "bg-destructive text-destructive-foreground" : "bg-muted text-muted-foreground border border-border"
                )}>
                  {order.category}
                </span>
              ...
              <div className="flex items-center justify-between md:justify-end gap-6 border-t md:border-t-0 pt-4 md:pt-0 border-border">
                <span className={cn(
                  ...
                  order.statusVariant === "danger" ? "text-destructive" : "text-foreground"
                )}>
                  {order.status}
                </span>
                <button 
                  onClick={() => handleRequest(order)}
                  className="px-6 h-9 text-[10px] font-black text-muted-foreground bg-muted border border-border rounded-md hover:bg-muted/80 transition-all uppercase tracking-widest outline-none"
                >
                  Raise Request
                </button>
              </div>
            ...
            <div className="space-y-4">
              <p className="text-[15px] md:text-[17px] font-bold text-foreground leading-relaxed">
                {order.description}
              </p>
              <div className="flex items-center gap-2 text-muted-foreground border-t border-border pt-4">
                ...
                <span className="text-[12px] font-bold">Ordered by: <span className="text-foreground">{order.doctor}</span></span>
              </div>
            </div>
          </div>
        ))}
