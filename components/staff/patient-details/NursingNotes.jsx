  ...
  return (
    <div className="space-y-[20px] animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-[18px] font-bold text-foreground leading-none">Nursing Notes</h3>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button 
              ...
              className="h-10 px-4 bg-transparent border border-border rounded-lg text-[12px] font-bold text-muted-foreground flex items-center gap-2 hover:bg-muted transition-all shadow-none"
            >
          ...
          <button 
            ...
            className="h-10 px-8 bg-primary text-primary-foreground rounded-lg text-[13px] font-bold hover:bg-primary/90 transition-all shadow-none"
          >
            Save Notes
          </button>
        </div>
      ...
      <div className="space-y-4">
        {filteredNotes.map((note, idx) => (
          <div key={idx} className="bg-card p-6 rounded-lg border border-border shadow-none space-y-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-0.5">
                    <p className="text-[14px] font-bold text-foreground">{note.author}</p>
                    <span className={cn(
                      "px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider",
                      note.roleVariant === "success" ? "bg-emerald-500/10 text-emerald-500" :
                      note.roleVariant === "purple" ? "bg-purple-500/10 text-purple-500" : "bg-muted text-muted-foreground"
                    )}>
                      {note.role}
                    </span>
                  </div>
                  <p className="text-[12px] font-medium text-muted-foreground">{note.time}</p>
                </div>
              </div>
              <button 
